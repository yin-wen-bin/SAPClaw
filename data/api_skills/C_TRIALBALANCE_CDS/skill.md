...
- If the user asks by profit center, select `ID`, `Ledger`, `CompanyCode`, `FiscalYear`, `FiscalPeriod`, `GLAccount`, `ProfitCenter`, `ProfitCenterName`, `Segment`, `SegmentName`, and balance amount fields; add `ProfitCenter ne ''` when no concrete profit center is supplied.
- If the user asks by segment, select `ID`, `Ledger`, `CompanyCode`, `FiscalYear`, `FiscalPeriod`, `GLAccount`, `Segment`, `SegmentName`, and balance amount fields; add `Segment ne ''` when no concrete segment is supplied.
