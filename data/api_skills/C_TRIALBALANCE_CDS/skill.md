...
- For wording such as `trial balance order balances`, `order balances for company code`, or `order balance dimension`, select only `C_TRIALBALANCEResults.Ledger`, `C_TRIALBALANCEResults.CompanyCode`, `C_TRIALBALANCEResults.GLAccount`, `C_TRIALBALANCEResults.OrderID`, and `C_TRIALBALANCEResults.EndingBalanceAmtInCoCodeCrcy`.
- For order balance dimension questions that ask for movements, also include debit/credit fields such as `DebitAmountInCoCodeCrcy` and `CreditAmountInCoCodeCrcy`.
