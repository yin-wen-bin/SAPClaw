# API_PARTNERCOMPANY_SRV Skill

## Purpose

Use this API for Trading Partner - Read. The service enables you to retrieve trading partner master data. It contains a header and nodes with business information. You can read trading partners and their texts.

Keep `data/index/API_PARTNERCOMPANY_SRV` as the schema ground truth. This skill provides business usage guidance only. This index was built from SAP metadata plus OpenAPI documentation.

## When To Use

- The user asks about trading partner - read or the business objects exposed by this service: A Partner Company.
- The request is read-oriented and can be answered from entity sets and fields indexed under `data/index/API_PARTNERCOMPANY_SRV`.
...
- `A_PartnerCompany`: Reads the items of all trading partners.. Methods: GET. runtime metadata available.

## Business Semantics

- Primary business scope: Trading Partner - Read.
- Use the service description, entity descriptions, and field descriptions from `data/index/API_PARTNERCOMPANY_SRV` to infer user intent.
...
- Verify entity and field availability against `data/index/API_PARTNERCOMPANY_SRV` before execution.
