# API_PAYMENT_ADVICE_SRV Skill

## Purpose

Use this API for Payment Advice (A2X). You can use this inbound synchronous service to create, read, update, and delete payment advices related to a customer or a supplier. It is based on the OData protocol and can be consumed in SAP Fiori apps and other user interfaces. The service can be used with payment advices of any supported type.

Keep `data/index/API_PAYMENT_ADVICE_SRV` as the schema ground truth. This skill provides business usage guidance only. This index was built from SAP metadata plus OpenAPI documentation.

## When To Use

- The user asks about payment advice (a2x) or the business objects exposed by this service: A Payment Advice Item, A Payment Advice.
- The request is read-oriented and can be answered from entity sets and fields indexed under `data/index/API_PAYMENT_ADVICE_SRV`.
...
- `A_PaymentAdviceItem`: Reads all item fields for all payment advice headers.. Methods: DELETE, GET, PATCH, POST. runtime metadata available.
- `A_PaymentAdvice`: Reads all available payment advices.. Methods: DELETE, GET, PATCH, POST. runtime metadata available.

## Business Semantics

- Primary business scope: Payment Advice (A2X).
- Use the service description, entity descriptions, and field descriptions from `data/index/API_PAYMENT_ADVICE_SRV` to infer user intent.
...
- Verify entity and field availability against `data/index/API_PAYMENT_ADVICE_SRV` before execution.
