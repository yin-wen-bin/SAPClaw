...
Use this API for This OData API enables you to create, change and read batches. Additionally, it is possible to retrieve and to edit the related classification data. The service is based on the OData protocol, and can be consumed by external systems and user interfaces.. This OData API enables you to create, change and read batches. Additionally, it is possible to retrieve and to edit the related classification data. The service is based on the OData protocol, and can be consumed by external systems and user interfaces.

Keep `data/index/API_BATCH_SRV` as the schema ground truth. This skill provides business usage guidance only. SAP $metadata is available in the local index; treat SAP 401/403/timeout responses as execution-time service or authorization issues, not schema absence.

## When To Use

- The user asks about this odata api enables you to create, change and read batches. additionally, it is possible to retrieve and to edit the related classification data. the service is based on the odata protocol, and can be consumed by external systems and user interfaces. or the business objects exposed by this service: Batch, Batch Charc, Batch Charc Value, Batch Class, Batch Plant, Batch Text.
- The request is read-oriented and can be answered from entity sets and fields indexed under `data/index/API_BATCH_SRV`.
...
- `Batch`: Get entities from Batch. Methods: GET, PATCH, POST.
- `BatchCharc`: Get entities from BatchCharc. Methods: GET, POST.
- `BatchCharcValue`: Get entities from BatchCharcValue. Methods: DELETE, GET, PATCH, POST.
- `BatchClass`: Get entities from BatchClass. Methods: DELETE, GET, POST.
- `BatchPlant`: Get entities from BatchPlant. Methods: GET, PATCH.
- `BatchText`: Get entities from BatchText. Methods: DELETE, GET, PATCH.

## Business Semantics

- Primary business scope: This OData API enables you to create, change and read batches. Additionally, it is possible to retrieve and to edit the related classification data. The service is based on the OData protocol, and can be consumed by external systems and user interfaces..
- Use the service description, entity descriptions, and field descriptions from `data/index/API_BATCH_SRV` to infer user intent.
...
- SAP $metadata was available when this index was built. Use `data/index/API_BATCH_SRV` as the executable schema contract.
...
- Verify entity and field availability against `data/index/API_BATCH_SRV` before execution.
