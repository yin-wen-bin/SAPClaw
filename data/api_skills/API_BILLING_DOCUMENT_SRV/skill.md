...
- A Runtime three-layer billing evidence plan binds header `BillingDocument` to item `BillingDocument`, then item `BillingDocument` and `BillingDocumentItem` to item pricing elements. Every binding source must set `fetch_all_for_binding=true`.
