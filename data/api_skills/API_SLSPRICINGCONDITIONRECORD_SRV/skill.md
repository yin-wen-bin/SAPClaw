# API_SLSPRICINGCONDITIONRECORD_SRV Skill

## Purpose

Use this API for Condition Record for Pricing in Sales. Consumers of this inbound service can create, read, update, and delete condition records inside condition record, validity of condition record, pricing scale, and condition supplement.

Keep `data/index/API_SLSPRICINGCONDITIONRECORD_SRV` as the schema ground truth. This skill provides business usage guidance only. This index was built from SAP metadata plus OpenAPI documentation.

## When To Use

- The user asks about condition record for pricing in sales or the business objects exposed by this service: A Sls Prcg Cndn Recd Suplmnt, A Sls Prcg Cndn Recd Validity, A Sls Prcg Condition Record, A Sls Prcg Condition Record Text, A Sls Prcg Cndn Record Scale, A Sls Prcg Cndn Supplement Text.
- The request is read-oriented and can be answered from entity sets and fields indexed under `data/index/API_SLSPRICINGCONDITIONRECORD_SRV`.
...
- `A_SlsPrcgCndnRecdSuplmnt`: Reads all condition supplements in the system.. Methods: DELETE, GET, PATCH, POST. runtime metadata available.
- `A_SlsPrcgCndnRecdValidity`: Reads validity of all condition records in the system.. Methods: GET, PATCH, POST. runtime metadata available.
- `A_SlsPrcgConditionRecord`: Reads all condition records in the system.. Methods: DELETE, GET, PATCH, POST. runtime metadata available.
- `A_SlsPrcgConditionRecordText`: Reads all pricing record texts in the system.. Methods: DELETE, GET, PATCH, POST. runtime metadata available.
- `A_SlsPrcgCndnRecordScale`: Reads all pricing scales in the system.. Methods: DELETE, GET, PATCH, POST. runtime metadata available.
- `A_SlsPrcgCndnSupplementText`: Reads all pricing supplement texts in the system.. Methods: DELETE, GET, PATCH, POST. runtime metadata available.

## Business Semantics

- Primary business scope: Condition Record for Pricing in Sales.
- Use the service description, entity descriptions, and field descriptions from `data/index/API_SLSPRICINGCONDITIONRECORD_SRV` to infer user intent.
...
- Verify entity and field availability against `data/index/API_SLSPRICINGCONDITIONRECORD_SRV` before execution.
