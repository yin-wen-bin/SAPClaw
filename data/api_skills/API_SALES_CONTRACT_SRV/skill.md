...
Keep `data/index/API_SALES_CONTRACT_SRV` as the schema ground truth. This skill provides business usage guidance only. This index was built from SAP metadata plus OpenAPI documentation.

## When To Use

- The user asks about sales contract (a2x) or the business objects exposed by this service: A Sales Contract, A Sales Contract Partner, A Sales Contract Prcg Elmnt, A Sales Contract Item, A Sales Contract Item Partner, A Sales Contract Item Prcg Elmnt.
- The request is read-oriented and can be answered from entity sets and fields indexed under `data/index/API_SALES_CONTRACT_SRV`.
...
- `A_SalesContract`: Reads all sales contract headers.. Methods: DELETE, GET, PATCH, POST. runtime metadata available.
- `A_SalesContractPartner`: Reads all header-level partners.. Methods: DELETE, GET, PATCH, POST. runtime metadata available.
- `A_SalesContractPrcgElmnt`: Reads all header-level pricing elements.. Methods: DELETE, GET, PATCH. runtime metadata available.
- `A_SalesContractItem`: Reads all sales contract items.. Methods: DELETE, GET, PATCH, POST. runtime metadata available.
- `A_SalesContractItemPartner`: Reads all item-level partners.. Methods: DELETE, GET, PATCH. runtime metadata available.
- `A_SalesContractItemPrcgElmnt`: Reads all item-level pricing elements.. Methods: DELETE, GET, PATCH. runtime metadata available.
- `A_SalesContractText`: Reads all header-level texts.. Methods: DELETE, GET, PATCH. runtime metadata available.
- `A_SalesContractItemText`: Reads all item-level texts.. Methods: DELETE, GET, PATCH. runtime metadata available.
- `A_SalesContractValAddedSrvc`: Reads all value-added services at item level.. Methods: DELETE, GET, PATCH, POST. documentation-only until SAP metadata is accessible.
- `rejectApprovalRequest`: Rejects a specific sales contract.. Methods: POST. documentation-only until SAP metadata is accessible.
- `releaseApprovalRequest`: Approves a specific sales contract.. Methods: POST. documentation-only until SAP metadata is accessible.

## Business Semantics

- Primary business scope: Sales Contract (A2X).
- Use the service description, entity descriptions, and field descriptions from `data/index/API_SALES_CONTRACT_SRV` to infer user intent.
...
- Verify entity and field availability against `data/index/API_SALES_CONTRACT_SRV` before execution.
