# API_PRODUCTGROUP_SRV Skill

## Purpose

Use this API for It contains product group and product group text nodes. This service is based on the OData protocol and can be consumed in Fiori apps and on other user interfaces.. It contains product group and product group text nodes. This service is based on the OData protocol and can be consumed in Fiori apps and on other user interfaces.

Keep `data/index/API_PRODUCTGROUP_SRV` as the schema ground truth. This skill provides business usage guidance only. SAP $metadata is available in the local index; treat SAP 401/403/timeout responses as execution-time service or authorization issues, not schema absence.

## When To Use

- The user asks about it contains product group and product group text nodes. this service is based on the odata protocol and can be consumed in fiori apps and on other user interfaces. or the business objects exposed by this service: Product Group, Product Group Text.
- The request is read-oriented and can be answered from entity sets and fields indexed under `data/index/API_PRODUCTGROUP_SRV`.
...
- `A_ProductGroup`: Get entities from A_ProductGroup. Methods: GET.
- `A_ProductGroupText`: Get entities from A_ProductGroupText. Methods: GET.

## Business Semantics

- Primary business scope: It contains product group and product group text nodes. This service is based on the OData protocol and can be consumed in Fiori apps and on other user interfaces..
- Use the service description, entity descriptions, and field descriptions from `data/index/API_PRODUCTGROUP_SRV` to infer user intent.
...
- SAP $metadata was available when this index was built. Use `data/index/API_PRODUCTGROUP_SRV` as the executable schema contract.
...
- Verify entity and field availability against `data/index/API_PRODUCTGROUP_SRV` before execution.
