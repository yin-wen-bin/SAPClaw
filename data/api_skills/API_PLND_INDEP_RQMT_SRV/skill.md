# API_PLND_INDEP_RQMT_SRV Skill

## Purpose

Use this API for The PIRs are read, updated, and/or created on header and item level. You can also send batch requests. Use this API if you want to work with the Change PIR or Maintain PIR apps.. The PIRs are read, updated, and/or created on header and item level. You can also send batch requests. Use this API if you want to work with the Change PIR or Maintain PIR apps.

Keep `data/index/API_PLND_INDEP_RQMT_SRV` as the schema ground truth. This skill provides business usage guidance only. SAP $metadata is available in the local index; treat SAP 401/403/timeout responses as execution-time service or authorization issues, not schema absence.

## When To Use

- The user asks about the pirs are read, updated, and/or created on header and item level. you can also send batch requests. use this api if you want to work with the change pir or maintain pir apps. or the business objects exposed by this service: Planned Indep Rqmt Item, Planned Indep Rqmt.
- The request is read-oriented and can be answered from entity sets and fields indexed under `data/index/API_PLND_INDEP_RQMT_SRV`.
...
- `PlannedIndepRqmtItem`: Reads Planned Independent Requirement Items.. Methods: GET, PATCH, POST.
- `PlannedIndepRqmt`: Reads Planned Independent Requirements.. Methods: GET, PATCH, POST.

## Business Semantics

- Primary business scope: The PIRs are read, updated, and/or created on header and item level. You can also send batch requests. Use this API if you want to work with the Change PIR or Maintain PIR apps..
- Use the service description, entity descriptions, and field descriptions from `data/index/API_PLND_INDEP_RQMT_SRV` to infer user intent.
...
- SAP $metadata was available when this index was built. Use `data/index/API_PLND_INDEP_RQMT_SRV` as the executable schema contract.
...
- Verify entity and field availability against `data/index/API_PLND_INDEP_RQMT_SRV` before execution.
