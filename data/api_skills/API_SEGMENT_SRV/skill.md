# API_SEGMENT_SRV Skill

## Purpose

Use this API for Segment - Read. The service enables you to retrieve segment master data. It contains a header and nodes with business information. You can read segments and their names in the respective language.

Keep `data/index/API_SEGMENT_SRV` as the schema ground truth. This skill provides business usage guidance only. This index was built from SAP metadata plus OpenAPI documentation.

## When To Use

- The user asks about segment - read or the business objects exposed by this service: A Segment, A Segment Text.
- The request is read-oriented and can be answered from entity sets and fields indexed under `data/index/API_SEGMENT_SRV`.
...
- `A_Segment`: Reads the items of all segments.. Methods: GET. runtime metadata available.
- `A_SegmentText`: Reads the texts of all segments.. Methods: GET. runtime metadata available.

## Business Semantics

- Primary business scope: Segment - Read.
- Use the service description, entity descriptions, and field descriptions from `data/index/API_SEGMENT_SRV` to infer user intent.
...
- Verify entity and field availability against `data/index/API_SEGMENT_SRV` before execution.
