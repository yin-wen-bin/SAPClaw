...
- For production order requests that explicitly ask for target/planned quantity or unit (`目标数量`, `计划数量`, `单位`), query `A_ProductionOrderItem_2` and select only `A_ProductionOrderItem_2.ManufacturingOrder`, `A_ProductionOrderItem_2.Material`, `A_ProductionOrderItem_2.ManufacturingOrderType`, `A_ProductionOrderItem_2.MfgOrderItemPlannedTotalQty`, and `A_ProductionOrderItem_2.ProductionUnit`; filter `A_ProductionOrderItem_2.Material` when the user provides a material.
