...
- For wording such as `日记账行项目`, `会计凭证项目`, or `财务行项目` without an explicit G/L-account, ledger, clearing-status, or open-item qualifier, prefer `A_JournalEntryItemBasic`. These are universal journal-entry business objects, not automatically G/L-account line-item requests.
...
- `A_JournalEntryItemBasic` is an analytical service: optional dimensions in `$select` change aggregation grain and total counts. For list/count comparisons, select only fields needed by the user's wording plus stable identifiers. When a matching pattern below says `select only`, its fields define the required grain; do not add name, ledger, cost/profit, functional-area, or other presentation dimensions unless the user explicitly asks for them.
