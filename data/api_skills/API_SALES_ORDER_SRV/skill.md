...
- Sales order item records are SD sales documents and belong to `API_SALES_ORDER_SRV.A_SalesOrderItem`; do not route sales order item wording to purchase order APIs.
...
- For wording such as `sales order records with their main identifying details`, `sales order main identifying details`, or `sales order header records`, select only `A_SalesOrder.SalesOrder`, `A_SalesOrder.SalesOrderType`, `A_SalesOrder.SalesOrganization`, `A_SalesOrder.DistributionChannel`, `A_SalesOrder.OrganizationDivision`, `A_SalesOrder.SalesGroup`, `A_SalesOrder.SalesOffice`, and `A_SalesOrder.SalesDistrict`.
- For wording such as `sales order item records with their main identifying details`, `sales order item records`, or `sales order line item records`, select only `A_SalesOrderItem.SalesOrder`, `A_SalesOrderItem.SalesOrderItem`, `A_SalesOrderItem.HigherLevelItem`, `A_SalesOrderItem.HigherLevelItemUsage`, `A_SalesOrderItem.SalesOrderItemCategory`, `A_SalesOrderItem.SalesOrderItemText`, `A_SalesOrderItem.PurchaseOrderByCustomer`, and `A_SalesOrderItem.PurchaseOrderByShipToParty`.
- For wording such as `sales order header pr element details`, `sales order header pricing element details`, or `sales order header condition elements`, select only `A_SalesOrderHeaderPrElement.PricingProcedureCounter`, `A_SalesOrderHeaderPrElement.PricingProcedureStep`, `A_SalesOrderHeaderPrElement.SalesOrder`, `A_SalesOrderHeaderPrElement.ConditionType`, `A_SalesOrderHeaderPrElement.PricingDateTime`, `A_SalesOrderHeaderPrElement.PriceConditionDeterminationDte`, `A_SalesOrderHeaderPrElement.ConditionCalculationType`, and `A_SalesOrderHeaderPrElement.ConditionBaseValue`.
- For wording such as `billing plan items for sales orders`, `sales order billing plan items`, or `sales order billing plan item records`, select only `A_SalesOrderBillingPlanItem.BillingPlan`, `A_SalesOrderBillingPlanItem.BillingPlanItem`, `A_SalesOrderBillingPlanItem.SalesOrder`, `A_SalesOrderBillingPlanItem.BillingPlanDateCategory`, `A_SalesOrderBillingPlanItem.BillingPlanBillingDate`, `A_SalesOrderBillingPlanItem.BillingPlanAmount`, `A_SalesOrderBillingPlanItem.TransactionCurrency`, and `A_SalesOrderBillingPlanItem.BillingPlanAmountPercent`.
...
- Use this shared read-only Thin path for exact sales-order status evidence across `API_SALES_ORDER_SRV`, `API_OUTBOUND_DELIVERY_SRV`, `API_BILLING_DOCUMENT_SRV`, and `API_OPLACCTGDOCITEMCUBE_SRV`.
...
- Set `fetch_all_for_binding=true` on every downstream binding source. The configured Thin binding-row safety limit still applies.
