...
from test_runtime import FakeExecutor, build_runtime, valid_plan


FIXTURE_ROOT = Path(__file__).resolve().parent / "fixtures" / "runtime_e2e"
...
def test_aggregate_rejects_top_on_any_execution_step() -> None:
    with pytest.raises(ValidationError, match="every execution step"):
        RuntimeQueryPlan.model_validate(
            {
...
def _supplier_aggregate_plan(**overrides) -> RuntimeQueryPlan:
    payload = {
        "service_name": "API_BUSINESS_PARTNER",
        "entity_set": "A_Supplier",
        "select_fields": ["Supplier"],
...
def _material_coverage_plan(*, include_required: bool) -> RuntimeQueryPlan:
    filters = [
...
def _o2c_plan() -> RuntimeQueryPlan:
    return RuntimeQueryPlan.model_validate(
        {
