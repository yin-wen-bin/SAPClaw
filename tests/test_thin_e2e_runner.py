...
from sap_odata_agent.tools.run_thin_codex_e2e import (
    baseline_retryable,
...
def test_prepare_suite_creates_stable_module_ids_and_preserves_natural_language(tmp_path: Path) -> None:
    ...
    assert [item["id"] for item in prepared] == ["THIN-FI-001", "THIN-FI-002"]
    ...
    assert all(item["test_surface"] == "codex_thin_mcp" for item in prepared)


def test_parse_json_object_accepts_fenced_or_embedded_json() -> None:
    assert parse_json_object('```json\n{"case_id":"thin-1"}\n```') == {"case_id": "thin-1"}
    assert parse_json_object('result: {"case_id":"thin-2"}') == {"case_id": "thin-2"}
...
def test_runtime_entry_payload_uses_saved_thin_audit_record() -> None:
    entry = {
    ...
    assert payload["execution_origin"] == "thin_mcp"
