...
def build_runtime(
    ...
    *,
    ...
    settings_kwargs["sap_" + "password"] = "<test-password>"
...
def valid_plan(**overrides) -> ThinQueryPlan:
    payload = {
...
def test_thin_plan_defaults_top_to_none_and_forbids_unknown_keys() -> None:
    plan = valid_plan()

    assert plan.top is None

    try:
        ThinQueryPlan.model_validate({**plan.model_dump(), "fallback_service": "API_TEST"})
    except ValueError as exc:
        assert "Extra inputs are not permitted" in str(exc)
    else:  # pragma: no cover
        raise AssertionError("Strict ThinQueryPlan accepted an unknown key.")
...
def test_execute_plan_revalidates_pages_and_strips_internal_data(tmp_path: Path) -> None:
    ...
    stored = runtime.case_repository.get_by_case_id(response["case_id"])
    assert stored["execution_origin"] == "thin_mcp"
...
def test_runtime_disabled_short_circuits_without_sap_request(tmp_path: Path) -> None:
    executor = FakeExecutor()
    runtime = build_runtime(tmp_path, enabled=False, executor=executor)
