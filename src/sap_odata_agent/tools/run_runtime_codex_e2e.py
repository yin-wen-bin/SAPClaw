...
from sap_odata_agent.tools.runtime_e2e_support import _compare, _expected_api_matches, _run_baseline


DEFAULT_MODULES = ("FI", "CO", "SD", "MM", "PP")
DEFAULT_SOURCE_ROOT = Path("data/cross_api_test_cases")
DEFAULT_CASE_ROOT = Path("data/runtime_test_cases")
DEFAULT_RUN_ROOT = Path("data/runtime_test_runs")
DEFAULT_SKILL_PATH = Path("skills/sapclaw-odata/SKILL.md")
DEFAULT_RUNTIME_BASE_URL = "http://127.0.0.1:8000"


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run deterministic SAP baselines and isolated Codex + SAPClaw Runtime MCP E2E cases."
    )
    parser.add_argument("--prepare", action="store_true", help="Create the 10-case-per-module Runtime suite.")
    parser.add_argument("--baseline-only", action="store_true", help="Run only deterministic SAP baselines.")
    parser.add_argument("--codex-only", action="store_true", help="Run only isolated Codex + SAPClaw Runtime MCP cases.")
    ...
    parser.add_argument(
        "--runtime-base-url",
        default=os.getenv("SAPCLAW_RUNTIME_BASE_URL", DEFAULT_RUNTIME_BASE_URL),
        help="Base URL for the isolated SAPClaw Runtime backend.",
    )
    ...
    run_id = args.run_id or datetime.now().strftime("runtime_%Y%m%d_%H%M%S")
    run_dir = Path(args.run_root) / run_id
    per_case_dir = run_dir / "per_case"
    per_case_dir.mkdir(parents=True, exist_ok=True)
    output_schema = write_output_schema(run_dir)
    workspace = Path("data/cases/runtime_codex_workspace")
    ...
    for module in modules:
        ...
        for index, source in enumerate(selected, start=1):
            prepared.append(
                {
                    **source,
                    "id": f"RUNTIME-{module}-{index:03d}",
                    "api": module,
                    "source_case_id": source.get("id"),
                    "test_surface": "codex_sapclaw_mcp",
                }
            )
...
def run_codex_case(
    *,
    ...
    prompt = (
        ...
        "Your final response must match the supplied JSON schema; set case_id to the SAPClaw Runtime case id.\n\n"
    ...
    runtime_case_id = str(parsed.get("case_id") or "")
    entry = get_case_repository().get_by_case_id(runtime_case_id) if runtime_case_id else None
    ...
    if not frontend:
        ...
        return {
            "success": False,
            "case_id": runtime_case_id or None,
...
def compare_case(
    ...
    *,
    ...
    if not frontend or frontend.get("execution_origin") != "sapclaw_mcp":
        return {
            "passed": False,
            "failed_layer": "runtime_audit",
            "reason": "Codex did not return a saved SAPClaw Runtime case.",
        }
...
def build_summary(run_id: str, results: list[dict[str, Any]], *, baseline_only: bool) -> dict[str, Any]:
    ...
    return {
        "run_id": run_id,
        "mode": "baseline_only" if baseline_only else "codex_sapclaw_mcp",
        "total": len(results),
        "passed": passed,
