...
class SapClawRuntimeClient:
    """HTTP client for the local, read-only SAPClaw Runtime endpoints."""
    ...
    def case_snapshot(self, case_id: str) -> JsonPayload:
        ...
        return self._request_json("GET", f"/api/v1/runtime/cases/{encoded_case_id}")
    ...
    def _request_json(self, method: str, path: str, payload: JsonPayload | None = None) -> JsonPayload:
        ...
        try:
            ...
            raise RuntimeClientError(
                "SAPClaw Runtime request timed out.",
                error_type="timeout",
                method=method,
                url=url,
            ) from exc
            ...
            raise RuntimeClientError(
                "SAPClaw Runtime is not reachable. Start the local FastAPI service first.",
        ...
        if status_code >= 400:
            raise RuntimeClientError(
                f"SAPClaw Runtime returned HTTP {status_code}.",
        ...
        if not isinstance(decoded, dict):
            raise RuntimeClientError(
                "SAPClaw Runtime returned an invalid JSON object.",
